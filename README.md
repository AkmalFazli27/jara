...
|:---|:---|:---|
| ~~**F-01**~~ | ~~Login multi-role untuk Pengguna dan Administrator.~~ *(Selesai)* | - ~~Form login menerima input email dan password yang valid~~<br>- ~~Pengguna dengan role `admin` diarahkan ke dashboard administrator~~<br>- ~~Pengguna dengan role `user` diarahkan ke dashboard proyek pengguna~~<br>- ~~Kredensial salah menampilkan pesan error validasi tanpa membocorkan info sistem~~ |
| ~~**F-02**~~ | ~~Logout dan proteksi rute privat melalui middleware auth.~~ *(Selesai)* | - ~~Tombol logout menghancurkan sesi aktif dan token auth~~<br>- ~~Rute privat tidak dapat diakses sebelum login (redirect otomatis ke login)~~<br>- ~~Mencegah akses halaman privat via browser back-button setelah logout~~ |
| ~~**F-03**~~ | ~~Lihat dan edit data profil pengguna.~~ *(Selesai)* | - ~~Halaman profil menampilkan nama, email, dan tanggal bergabung~~<br>- ~~Pengguna dapat memperbarui nama dan password lama ke password baru~~<br>- ~~Validasi email unik tetap terjaga saat update profil~~ |
| ~~**F-04**~~ | ~~Admin melihat daftar seluruh pengguna terdaftar di sistem.~~ *(Selesai)* | - ~~Tabel menampilkan ID, Nama, Email, Role (Admin/User), dan Status Akun~~<br>- ~~Tersedia fitur pencarian dan paginasi data pengguna~~<br>- ~~Hanya dapat diakses oleh pengguna dengan role `admin`~~ |
| ~~**F-05**~~ | ~~Admin menambahkan akun pengguna baru secara manual.~~ *(Selesai)* | - ~~Form tambah akun memvalidasi format email, nama, dan password default~~<br>- ~~Password dienkripsi menggunakan bcrypt sebelum disimpan ke database~~<br>- ~~Akun baru langsung aktif dan dapat digunakan untuk login~~ |
| ~~**F-06**~~ | ~~Admin menghapus atau menonaktifkan akun pengguna.~~ *(Selesai)* | - ~~Tersedia tombol toggle status (Active/Inactive) dan tombol hapus akun~~<br>- ~~Akun yang berstatus nonaktif ditolak saat mencoba login~~<br>- ~~Admin tidak dapat menghapus akun miliknya sendiri (self-delete protection)~~ |
